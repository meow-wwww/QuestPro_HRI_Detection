...
public class EXPDogOperation : MonoBehaviour
{
        ...
        if (table == null && globalPositionInfo.GlobalPositionSet){
            ...
            if (table != null){
                ...
                if (globalPositionInfo.sceneName == "Sitting"){
                    ...
                    user1Near = new Vector3(table.transform.position.x, globalPositionInfo.floorHeight, table.transform.position.z) + 0.7f * globalPositionInfo.userRight - 0.55f * globalPositionInfo.userForward;
    ...
    public void SendOutDrink(bool dangerous=false){
        ...
        Debug.Log("+++++++++++++++++ cupToTableHeight: " + cupToTableHeight);
            ...
            StartCoroutine(
                controller.WaitForCoroutinesToEnd(new List<IEnumerator>(){
                    gameObject.GetComponent<ExecuteMovement>().MoveAlongPath_ROS_Coroutine(
                        new List<Vector3>{}, // rotate to face the table
                        moveSpeed, rotateSpeed,
                        finalRotate: true,
                        finalFaceTowards: table.transform.position
                    ),
                    gameObject.GetComponent<DogNotification>().SendVoiceRequest_Coroutine("Ding"),
                    controller.Arm1LengthChange(cupToTableHeight + additionalHeight, 1),
                    controller.Arm2LengthChange(sendOutDrinkDistance, 1),
                    controller.Arm1LengthChange(additionalHeight, -1),
                    CurrentDrinkDetach(),
    ...
    public void MoveToTableUser1FromCollision_Fixed(){
        if (globalPositionInfo.sceneName == "Sitting"){
            StartCoroutine(gameObject.GetComponent<ExecuteMovement>().MoveAlongPath_ROS_Coroutine(
                new List<Vector3>{
                    user1Peripheral,
                    user1Near
                }, 
        ...
        StartCoroutine(
            controller.WaitForCoroutinesToEnd(
                new List<IEnumerator>(){
                    ...
                    executor.MoveAlongPath_ROS_Loop_Coroutine(
                        new List<Vector3>{
